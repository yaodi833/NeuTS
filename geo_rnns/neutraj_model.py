...
class RNNEncoder(Module):
    def __init__(self, input_size, hidden_size, grid_size, stard_LSTM= False, incell = True):
        ...
        if self.stard_LSTM:
            if config.recurrent_unit=='GRU':
                self.cell = torch.nn.GRUCell(input_size - config.dimensional, hidden_size).cuda()
            elif config.recurrent_unit=='SimpleRNN':
                self.cell = torch.nn.RNNCell(input_size - config.dimensional, hidden_size).cuda()
            else:
                self.cell = torch.nn.LSTMCell(input_size - config.dimensional, hidden_size).cuda()
        else:
            if config.recurrent_unit=='GRU':
    ...
    def forward(self, inputs_a, initial_state = None):
        ...
        for t in range(time_steps):
            if self.stard_LSTM:
                cell_input = inputs[:, t, :][:,:-config.dimensional]
            else:
                cell_input = inputs[:, t, :]
            if config.recurrent_unit == 'GRU' or config.recurrent_unit == 'SimpleRNN':
