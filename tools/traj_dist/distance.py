from tools.traj_dist.pydist.erp import e_erp, g_erp
import numpy
import pyximport
pyximport.install(setup_args={"include_dirs": numpy.get_include()},
                  reload_support=True)
from tools.traj_dist.pydist.linecell import trajectory_set_grid

from tools.traj_dist.pydist.sspd import e_sspd, g_sspd
from tools.traj_dist.pydist.dtw import g_dtw
from tools.traj_dist.pydist.dtw_c import e_dtw, e_cdtw
from tools.traj_dist.pydist.erp import g_erp
from tools.traj_dist.pydist.erp_c import e_erp
from tools.traj_dist.pydist.edr import e_edr, g_edr
from tools.traj_dist.pydist.lcss import e_lcss, g_lcss
from tools.traj_dist.pydist.frechet import frechet
from tools.traj_dist.pydist.discret_frechet_c import discret_frechet
from tools.traj_dist.pydist.hausdorff import e_hausdorff, g_hausdorff
from tools.traj_dist.pydist.sowd import sowd_grid
...
METRIC_DIC = {"geographical": {"cython": {},
                               "python": {"sspd": g_sspd,
                                          "dtw": g_dtw,
                                          "lcss": g_lcss,
...
def cdtw(traj_1, traj_2, w, type_d="euclidean", implementation="auto"):
    """
    Usage
    -----
    Compute the Dynamic Time Warping Distance between trajectory traj_1 and traj_2
    ...
    if dim_1 != dim_2:
        raise ValueError(
            "Trajectories should have same dimensions. %d and %d given" % (dim_1, dim_2))
    dim = dim_1

    if not (type_d in ["geographical", "euclidean"]):
        raise ValueError(
            "The type_d argument should be 'euclidean' or 'geographical'\ntype_d given is : " + type_d)
    ...
    dist_func = METRIC_DIC[type_d][implementation]["cdtw"]
    dist = dist_func(traj_1, traj_2, w)
    return dist
...
def cdist(traj_list_1, traj_list_2, metric="sspd", type_d="euclidean", implementation="auto", converted = None, precision = None,
    ...
    if not (metric in ["sspd", "dtw", "lcss", "hausdorff", "frechet", "discret_frechet", "sowd_grid","cdtw",
                       "erp", "edr"]):
        raise ValueError("The metric argument should be 'sspd', 'dtw', 'lcss','erp','edr' 'hausdorff', 'frechet',"
                         "'discret_frechet' or 'sowd_grid'\nmetric given is : " + metric)
    ...
    if metric.startswith("sowd_grid"):
        ...
        for i in range(nb_traj_1):
            ...
            for j in range(nb_traj_2):
                ...
                # print(j, M[i, j])
    elif metric == "lcss" or metric == "edr" or metric == "cdtw":
        if eps is None:
            warnings.warn("eps parameter should be specified for metric 'lcss' and 'edr', default is 100 ")
            eps=100
        for i in range(nb_traj_1):
